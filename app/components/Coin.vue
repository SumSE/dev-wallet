    ...
    <article class="tile is-child box">
        ...
        <div class="level">
            <div class="level-item has-text-centered">
                <div>
                    ...
                    <p class="title">{{ balance.toLocaleString() }}</p>
                </div>
            </div>
        </div>
        <div class="level">
            <div class="level-item has-text-centered">
                <div>
                    ...
                    <div class="columns" v-if="address">
                        <div class="column">
                            <button class="button is-primary" @click="openSendModal">
        ...
        <b-table :data="transactions">
            <template slot-scope="props">
                <b-table-column field="date" label="Date" width="120" centered>
                    {{ props.row.date.toDate().toLocaleDateString('ja-JP', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) }}
                </b-table-column>

                <b-table-column label="Type" width="80" centered>
                    <span class="tag is-success">
                        {{ props.row.type == 1 ? 'send' : 'receive' }}
                    </span>
                </b-table-column>

                <b-table-column field="from_to" label="From/To" centered class="from_to">
                    {{ props.row.from_to }}
                </b-table-column>

                <b-table-column field="name" label="Name" centered class="from_to">
                    {{ props.row.name }}
                </b-table-column>

                <b-table-column field="value" label="Value" width="100" numeric>
...
export default {
    ...
    computed: {
        ...
        balance () {
            return this.$store.getters[this.type + '/balance']
        },
        transactions () {
